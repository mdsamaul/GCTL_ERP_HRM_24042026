...
namespace GCTL.Service.ProductIssueEntrys
{
    public interface IProductIssueEntryService
    {
        ...
        Task<(bool isSuccess, string message, object data)> CreateUpdateDetailsAsync(ProductIssueInformationDetailViewModel model, string companyCode);
        Task<(bool isSuccess, string message, object data)> DeleteAsync(List<string> ids);
        Task<string> AutoProductIssueEntryIdAsync();
