(function ($) {
    ...
    <tr class="data-row" data-id="${item.id || 0}">
        ...
        <td>
            <input type="text" class="form-control-sm form-control description" value="${item.description || ''}" placeholder="Enter description">
        </td>
        <td>
            <input type="text" class="form-control-sm form-control text-center width" value="${item.width || ''}" placeholder="e.g., 100cm">
        </td>
        <td>
        ...
        <td>
            <input type="text" class="form-control-sm form-control text-center po-no" value="${item.poNo || ''}" placeholder="PO Number">
        </td>
        <td>
            <input type="number" class="form-control-sm form-control text-center gar-qty" value="${item.quantity || 0}" placeholder="0">
        </td>
        <td>
            <input type="number" step="0.01" class="form-control-sm form-control text-center cons-pcs" value="${item.consumption || 0}" placeholder="0.00">
        </td>
        <td>
            <input type="number" step="0.01" class="form-control-sm form-control text-center ex-percent" value="${item.extra || 0}" placeholder="0.00">
        </td>
        <td>
        ...
        <td>
            <input type="number" step="0.01" class="form-control-sm form-control text-center unit-price" value="${item.unitPrice || 0}" placeholder="0.00">
        </td>
        <td>
        ...
        //    // Excel file globally store 
        //    selectedExcelFile = e.target.files[0];

        //    var file = selectedExcelFile;
        //    if (!file) return;
       ...
       // $(document).on('click', '#excelPreviewBtn', function () {
            ...
            //// Excel preview data fetch 
            //$.ajax({
            //    url: commonName.baseUrl + '/PreviewExcel',
            //    method: 'POST',
            //    data: formData,
            //    processData: false,
            //    contentType: false,
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        setTimeout(() => {
                            ...
                            //$(`#commonModal .modal-body`).find('.searchable-select').each(function () {
                            //    const $select = $(this);
                            //    debugger
                            ...
                            //    // CRITICAL: Skip if this select is inside a nested modal
                            //    const $closestModal = $select.closest('.modal');
                            //    if ($closestModal.attr('id') !== 'commonModal') {
                            //        return;
                            //    }

                            //    // Skip if this element is not visible (might be in a nested/hidden modal)
                            //    if (!$select.is(':visible') && $select.closest(`#commonModal`).length === 0) {
                            //        return;
                            //    }
        ...
        function RmgCostingInfoDto() {
            ...
            const parseDate = (val) => {
                if (!val) return null;
                // Assuming input format is dd/MM/yyyy or yyyy-MM-dd
        ...
        $(document).on('click', ".js-costing-info-save", function () {
            ...
            //    showToast("error", "P.O is required.");
            //    $('#poInput').addClass('border-danger');
            //    return;
            //}
        ...
        //    let productId = $("#productInput").val().trim();

        //    if (!costingId || !integraJobNo || !purchaseOrder || !productId) {
        //        alert("Please enter all required fields: Costing ID, Integra Job No, Purchase Order, and Product ID");
        //        return;
        //    }
        ...
        //        error: function (xhr) {
        //            if (xhr.status === 404) {
        //                alert("Costing report not found");
        //            } else {
        //                alert("Error loading costing report");
        //            }
        //        }
        //    });
        //});
        ...
        function generatePdf(data) {
            ...
            summaryRows.forEach((row, idx) => {
                ...
                if (row.type === 'bordered' || row.type === 'bordered-total') {
                    ...
                    // Column 11 - position adjust এবং width কমানো
